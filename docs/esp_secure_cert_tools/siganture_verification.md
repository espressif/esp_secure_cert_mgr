...
- This struct includes fileds
    - `abort_on_failure`: if set `true` then aborts the device on failure. If `NULL` is passed to API or abort_on_failure is set to `false` then API just return `ESP_FAIL` status in failure.
