...
void app_main()
{
    ...
    esp_ret = esp_secure_cert_tlv_get_addr(ESP_SECURE_CERT_USER_DATA_1, 0, &addr, &len);
    if (esp_ret == ESP_OK) {
        ESP_LOG_BUFFER_HEX(TAG, addr, len);
    }

    esp_ret = esp_secure_cert_tlv_get_addr(ESP_SECURE_CERT_DEV_CERT_TLV, 1, &addr, &len);
    if (esp_ret == ESP_OK) {
        printf("Device Cert: \n%s", addr);
    }

    esp_ret = esp_secure_cert_tlv_get_addr(ESP_SECURE_CERT_PRIV_KEY_TLV, 1, &addr, &len);
    if (esp_ret == ESP_OK) {
        printf("Priv Key: \n%s", addr);
    }
